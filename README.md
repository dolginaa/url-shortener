...
## Архитектура, паттерны и техники

### Архитектура

- **Слоистая архитектура (Layered / Clean Architecture)**  
  Чёткое разделение на слои: `cmd` → `delivery` → `usecase` → `domain` и `infrastructure`. Зависимости направлены внутрь.

- **Разделение internal / pkg**
